...
export function DebtPayoffSheet({ onClose }: { onClose: () => void }) {
  const { debt, recordCardAttempt } = useStore();
  ...
  function handlePay(e: React.FormEvent) {
    ...
    if (!isPlausibleExpiry(expiry)) {
      setCardError('Expiration date is invalid.');
      return;
    }
    recordCardAttempt({ last4Hash: hashLast4(raw), amount, context: 'iou-payoff' });
    setShowBerate(true);
  }
